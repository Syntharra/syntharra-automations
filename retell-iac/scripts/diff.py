...
def norm(x):
    if isinstance(x, dict):
        return {k:norm(v) for k,v in sorted(x.items()) if k not in IGNORE_KEYS and not k.endswith("_timestamp")}
    if isinstance(x, list):
        items=[norm(i) for i in x]
        # Sort by deterministic key if possible
        try:
...
def deep_diff(a, b, path=""):
    ...
    if isinstance(a, dict):
        for k in set(a)|set(b):
            if k not in a: diffs.append((f"{path}.{k}","missing-in-a",None,b[k]))
            elif k not in b: diffs.append((f"{path}.{k}","missing-in-b",a[k],None))
...
if __name__=="__main__":
    ...
    args=p.parse_args()
    A=norm(json.load(open(args.a)))
    B=norm(json.load(open(args.b)))
    d=deep_diff(A,B)
    if not d:
