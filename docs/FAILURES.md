...
| 2026-04-04 | Simulator | Groq 12k TPM hit — premium prompt 24k chars vs standard 10k | Node instructions bloating prompt 2.5x. 12 calls/scenario × 6k tokens = rate limit every scenario | Strip node instructions from fetch_agent_prompt() — global prompt only (10k chars). Same fix applied to both simulators | No |
| 2026-04-04 | Simulator | llama-4-scout swap unnecessary complexity | TPM limit was the real issue, not the model | Reverted to llama-3.3-70b-versatile (same as standard). Root fix = smaller prompt | No |
