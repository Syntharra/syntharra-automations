...
- **Re-register MASTER agent in Retell** — MASTER (`agent_4afbfdb3fcb1ba9569353af28d`) and TESTING (`agent_6e7a2ae03c2fbd7a251fafcd00`) were deleted 2026-04-09 during Retell tidy-up. Recreate from retell-iac snapshot: (1) POST `retell-iac/snapshots/2026-04-09_testing-autolayout-fixed/flow.json` to Retell `create-conversation-flow`, get new flow_id; (2) POST agent shape from `agent.json` (omit `agent_id` field) to `create-agent`, get new agent_id; (3) update `docs/REFERENCE.md`, `docs/STATE.md`, `retell-iac/CLAUDE.md` with new IDs; (4) update `syntharra_vault` Retell AI entries; (5) wire `post_call_webhook_url`. See FAILURES.md 2026-04-09 row.
- **Build `tools/usage_alert.py`** — 80%/100% mid-month usage alert emails to clients. Old n8n workflow `Wa3pHRMwSjbZHqMC` is broken (queries dropped `hvac_call_log`). New script: daily run, Retell `list-calls` for current month, check `billing_cycles.alert_80_sent` / `alert_100_sent`, send Brevo email at threshold, write `billing_cycles` row. See STATE.md note.
