...
> Every session must get smarter than the last. This section governs how Claude learns from mistakes and updates its behavior — automatically, within each session.

### At session START (always)

1. Read `docs/SESSION_START.md` top-to-bottom (entry point for all sessions).
2. In order: `docs/STATE.md` → `docs/RULES.md` → `docs/REFERENCE.md` → `docs/FAILURES.md` (last 10 rows).
3. Run `python tools/session_start.py` for the 15-line orientation block.
4. Check Claude memory index at `~/.claude/projects/.../memory/MEMORY.md` for session-relevant entries.
5. For any task that touches a system listed in the last 5 FAILURES.md rows — read the full failure row before starting.

### During tasks (continuous)

- **IDs only from `docs/REFERENCE.md`.** Never recall from memory. Never inline elsewhere.
- **Never test on live Retell agents.** Clone → TESTING → iterate → `promote.py` → live.
- **Credentials always from `syntharra_vault`.** Zero hardcoded secrets in any file.
- **n8n workflow changes via REST API only.** Never use n8n MCP for targeted patches (it rewrites the full workflow).
- **Before any email variable change**, trace the full path: webhook payload → Set node key → HTML template.

### When something breaks (immediate)

Load `skills/self-improvement-SKILL.md` and follow the 6-step protocol:
1. Capture the failure (symptom, root cause, fix, rule implied)
2. Append to `docs/FAILURES.md`
3. Update `docs/RULES.md` if a standing rule is implied
4. Update Claude memory file `feedback_anti_patterns.md`
5. Run `python tools/analyze_failures.py` to check for hotspot patterns
6. Patch the relevant skill file with a GOTCHA entry if domain knowledge was missing

### At session END (always)

```bash
python tools/session_end.py --topic <slug> --summary "<one-line>"
```

### Anti-patterns (learned from Syntharra incidents)

| Never do this | Do this instead | Source |
|---|---|---|
| Direct API PATCH on MASTER Retell agent | `retell-iac/` → `promote.py` | RULES.md §1 |
| Inline an agent/flow/workflow ID | Read from `docs/REFERENCE.md` | RULES.md §2 |
| Write per-client files into the repo | Store in Supabase `client_agents` | RULES.md §3 |
| Use n8n MCP to patch a single node | REST API: read → modify → PUT | FAILURES.md 2026-04-08 |
| Guess variable key names in email templates | Trace full path: webhook → Set → HTML | FAILURES.md 2026-04-08 |
| Hardcode API keys/tokens in workflows | Query `syntharra_vault` at runtime | FAILURES.md 2026-04-08 |
| Query `public.vault` or filter on `active` | `syntharra_vault WHERE service_name='X'` | FAILURES.md 2026-04-08 |
| Leave `sendHeaders=false` on auth'd HTTP nodes | Set `sendHeaders: true` explicitly | FAILURES.md 2026-04-07 |
| Retry GitHub MCP 403 in same subagent | Fresh subagent → Desktop Commander | RULES.md §4 |
...
| Post-task review / failure logging | `skills/self-improvement-SKILL.md` |
