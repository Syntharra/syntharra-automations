# HVAC Standard Agent — Prompt Export
...
**Edges:**
- → `node-leadcapture` (Repair, service, maintenance, tune-up, inspection)
- → `node-leadcapture` (Quote, estimate, installation, replacement, new system)
- → `node-verify-emergency` (Emergency, urgent, no cooling, no heat, water leak, burning smell, gas smell)
- → `node-callback` (Returning a missed call, got a call from this number, calling back)
- → `node-existing-customer` (Existing customer, question about appointment, invoice, technician, job already booked)
- → `node-general-questions` (General question about services, hours, pricing, area, credentials)
- → `node-transfer-call` (Wants to speak to a real person, manager, owner, or specific staff member)
- → `node-spam-robocall` (Robocall, spam, automated message, silence after greeting)
...
**Edges:**
- → `node-verify-emergency` (Emergency signals detected)
- → `node-ending` (All details confirmed, close)
...
**Edges:**
- → `node-transfer-call` (Confirmed emergency, transfer now)
- → `node-leadcapture` (Urgent but not emergency, capture as priority lead)
...
**Edges:**
- → `node-ending` (Details captured, close)
...
**Edges:**
- → `node-ending` (Enquiry noted, close)
- → `node-transfer-call` (Caller insists on speaking to someone now)
...
**Edges:**
- → `node-ending` (Question answered, no booking needed)
- → `node-leadcapture` (Caller wants to book or get a quote)
- → `node-transfer-call` (Caller needs further help beyond FAQ)
...
**Edges:**
- → `node-ending` (Details taken, close)
...
**Edges:**
- → `node-end-call` (Nothing else. Close: "Have a great day, take care!")
- → `node-identify-call` (Caller has another question or request)
