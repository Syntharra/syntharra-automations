...
if flow_id:
    ...
    check("flex_mode off (rigid mode)",   flow.get('flex_mode') in [False, None],                 str(flow.get('flex_mode')) + " (None=rigid by default)")
    ...
    check("Greeting has correct greeting",bool(gt) and TEST_AGENT in gt,                           gt[:60])
    check("Greeting contains agent name", TEST_AGENT in gt,                                        gt[:60])
    check("Global prompt contains company info", TEST_COMPANY in flow.get('global_prompt', ''))
    # Publish
    ...
    fake_call = {
        ...
        "call": {
            ...
            "transcript": (
                ...
                "Agent: I'm sorry to hear that. What can I help you with today? "
                "Caller: I need someone to come look at my furnace, it won't turn on. "
                "Agent: No problem at all. Can I get your full name please? "
                "Caller: Daniel Blackmore. "
                "Agent: D-A-N-I-E-L B-L-A-C-K-M-O-R-E, is that right? "
                "Caller: Yes that's correct. "
                "Agent: And your best callback number? "
                "Caller: 631-633-0713. "
                "Agent: So that is 631, 633, 0713, is that right? "
                "Caller: Yes. "
                "Agent: And the service address? "
                "Caller: 45 Park Avenue, Manhattan, New York. "
                "Agent: Just to confirm, that is 45 Park Avenue, Manhattan, is that right? "
                "Caller: Yes correct. "
                "Agent: Any other details I should pass on? "
                "Caller: It started making a loud noise yesterday then just stopped. "
                "Agent: Perfect, I have passed all of that through to the team and someone will be in touch with you shortly."
            )
        }
    }
...
# ══════════════════════════════════════════════════════════════
print("\n[7] EMAIL CHECK (manual)")
# ══════════════════════════════════════════════════════════════
check("Onboarding email — check daniel@syntharra.com inbox", exec_status == 'success',
    "Should have received 'New HVAC Agent Created' email")
